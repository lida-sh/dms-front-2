<template>
    <div class="flex flex-col lg:flex-row  items-start w-full gap-4 px-10 py-14 bg-[#F7F7F7] select-none h-screen">
          ...
          <div class="divide-y w-full">
            <div class="p-3">
              <app-select-input name="architecture_id" :options="architectures!" :label="$t('architecture_id')"
                class="w-full" @selectedItem="(architecture) => handleFilter({ architecture_id: architecture })"></app-select-input>
            </div>
            <!-- <div class="p-3">
      ...
      <section class="w-full h-full flex">
        <div class="px-4 w-full h-full flex flex-col items-start gap-4">
          <h1 class="font-sm xl:text-base font-bold mb-4 mr-4">فرایندهای مخابرات ایران :</h1>
          <process-result-search v-for="(itemDoc, index) in data?.processes" :key="index" :rowNumber="((data!.meta.current_page - 1) * data!.meta.per_page) + index + 1"
  ...
  import { useGetBaseArchitecturesService, useGetBaseProcessesService, useGetProceduresService, useGetProcessesService } from '~/composables/home/home.service'
